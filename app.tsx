  ...
  return <section className="provider-card">
    ...
    {provider.switchable && <div className="provider-accounts">
      ...
      <div className="provider-account-list">{provider.accounts.map((account) => <div key={account.id} className="provider-account-row">
        <div className="provider-account-name"><Icon name={account.active ? "CircleCheck" : "Circle"} className={account.active ? "size-4 text-emerald-600" : "size-4 text-muted-foreground"}/><span>{account.label}</span></div>
        ...
        <Button size="icon" variant="ghost" disabled={busy !== null} onClick={() => run(`remove:${provider.id}:${account.id}`, () => rpc.call("accounts_remove", { provider: provider.id as SwitchableProviderId, id: account.id }))} aria-label={`Remove ${account.label}`}><Icon name="Trash2" className="size-4"/></Button>
      </div>)}</div>
      ...
      {provider.id === "codex" && <p className="provider-note">Codex switching always reads and writes <code>~/.codex/auth.json</code>.</p>}
